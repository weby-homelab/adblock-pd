...
// options contains all command-line options for the ADBlock-PD(.exe) binary.
type options struct {
	...
	//
	//   - "install":  Installs ADBlock-Private-DNS as a system service.
	...
	// checkConfig, if true, instructs ADBlock-Private-DNS to check the configuration
	// file, optionally print an error message to stdout, and exit with a
	// corresponding exit code.
	checkConfig bool

	// disableUpdate, if true, prevents ADBlock-Private-DNS from automatically checking
	// for updates.
	//
	// TODO(a.garipov): Use.
	disableUpdate bool
	...
	// help, if true, instructs ADBlock-Private-DNS to print the command-line option
	// help message and quit with a successful exit-code.
	help bool

	// localFrontend, if true, instructs ADBlock-Private-DNS to use the local frontend
	// directory instead of the files compiled into the binary.
	//
	// TODO(a.garipov): Use.
	localFrontend bool

	// performUpdate, if true, instructs ADBlock-Private-DNS to update the current
	// binary and restart the service in case it's installed.
	//
	// TODO(a.garipov): Use.
	performUpdate bool

	// noPermCheck, if true, instructs ADBlock-Private-DNS to skip checking and
	// migrating the permissions of its security-sensitive files.
	//
	// TODO(e.burkov):  Use.
	noPermCheck bool

	// verbose, if true, instructs ADBlock-Private-DNS to enable verbose logging.
	verbose bool

	// version, if true, instructs ADBlock-Private-DNS to print the version to stdout
...
var commandLineOptions = []*commandLineOption{
	confFileIdx: {
		...
		defaultValue: "internal/next/ADBlock-PD.yaml",
...
// parseOptions parses the command-line options for ADBlock-PD.
...
// processOptions decides if ADBlock-Private-DNS should exit depending on the results
...
func processOptions(
	...
	if opts.version {
		if opts.verbose {
			...
			fmt.Printf("ADBlock-Private-DNS %s\n", version.Version())
		}
