...
type options struct {
	...
	// workDir is the path to the working directory where ADBlock-Private-DNS stores
	// filter data, the query log, and other data.
	workDir string

	// logFile is the path to the log file.  If empty, ADBlock-Private-DNS writes to
	...
	// disableUpdate, if set, makes ADBlock-Private-DNS not check for updates.
	disableUpdate bool

	// performUpdate, if set, updates ADBlock-Private-DNS without GUI and exits.
	performUpdate bool
	...
	// localFrontend forces ADBlock-Private-DNS to use the frontend files from disk
...
// cmdLineOpts are all command-line options of ADBlock-Private-DNS.
