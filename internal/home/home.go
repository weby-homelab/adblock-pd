// Package home contains ADBlock-Private-DNS's HTTP API methods.
package home
...
func setupDNSFilteringConf(
	...
	const (
		...
		defaultSafeBrowsingServer = "127.0.0.1"
		sbTXTSuffix               = ""

		pcService             = "parental_control"
		defaultParentalServer = "127.0.0.1"
		pcTXTSuffix           = ""
	)
...
func run(
	...
	initUpdate(ctx, baseLogger, opts, tlsMgr, isFirstRun, workDir, confPath)
...
func initUpdate(
	...
	_ string,
	_ string,
) (upd interface{}, isCustomURL bool) {
	return nil, false
}
...
// used by ADBlock-Private-DNS, if needed.
...
ADBlock-Private-DNS is not allowed to bind to privileged ports (for instance, port 53).
...
func checkNetworkPermissions(ctx context.Context, l *slog.Logger) {
	...
	// We should check if ADBlock-Private-DNS is able to bind to port 53
...
func initConfigFilename(
	...
	confPath = filepath.Join(workDir, "ADBlock-PD.yaml")
...
func cleanup(ctx context.Context) {
	log.Info("stopping ADBlock-Private-DNS")
...
// detectFirstRun returns true if this is the first run of ADBlock-Private-DNS.  l must
// not be nil.
