...
func encodeMobileConfig(d *dnsSettings, clientID string) ([]byte, error) {
	...
	data := &mobileConfig{
		PayloadDescription: "Adds ADBlock-Private-DNS to macOS Big Sur and iOS 14 or newer systems",
		...
		PayloadContent: []*payloadContent{{
			...
			PayloadDisplayName: dspName,
			PayloadDescription: "Configures device to use ADBlock-Private-DNS",
