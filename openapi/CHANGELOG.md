# ADBlock-Private-DNS API Change Log
...
- The new field `"upstreams_cache_enabled"` in `GET /control/clients`, `GET /control/clients/find`, `POST /control/clients/add`, and `POST /control/clients/update` methods shows if client’s DNS cache is enabled for the client.  If not set ADBlock-Private-DNS will use default value (false).
...
- The new optional fields `"ignore_querylog"` and `"ignore_statistics"` are set if ADBlock-Private-DNS excludes client activity from query log or statistics.

### `POST /control/clients/add` And `POST /control/clients/update`

- The new optional fields `"ignore_querylog"` and `"ignore_statistics"` make ADBlock-Private-DNS exclude client activity from query log or statistics.  If not set ADBlock-Private-DNS will use default value (false).  It can be changed in the future versions.

## v0.107.27: API changes

### The new optional fields `"edns_cs_use_custom"` and `"edns_cs_custom_ip"` in `DNSConfig`

- The new optional fields `"edns_cs_use_custom"` and `"edns_cs_custom_ip"` in `POST /control/dns_config` method makes ADBlock-Private-DNS use or not use the custom IP for EDNS Client Subnet.

- The new optional fields `"edns_cs_use_custom"` and `"edns_cs_custom_ip"` in `GET /control/dns_info` method are set if ADBlock-Private-DNS uses custom IP for EDNS Client Subnet.
...
They never quite worked properly, and the future new version of ADBlock-Private-DNS API will probably be different.
...
As an additional CSRF protection measure, ADBlock-Private-DNS now ensures that requests that change its state but have no body do not have a `Content-Type` header set on them.
...
- The new field `"version"` in `GET /install/get_addresses` is the version of the ADBlock-Private-DNS instance.
...
- The new optional field `"cache_optimistic"` in `POST /control/dns_config` method makes ADBlock-Private-DNS use or not use the optimistic cache mechanism.

- The new field `"cache_optimistic"` in `GET /control/dns_info` method is true if ADBlock-Private-DNS uses the optimistic cache mechanism.
...
- The new optional field `"default_local_ptr_upstreams"` is the list of IP addresses ADBlock-Private-DNS would use by default to resolve PTR request for addresses from locally-served networks.
...
[Identifying clients]: https://github.com/AdguardTeam/ADBlock-PD/wiki/Clients#idclient
...
- Added optional "offset" and "limit" parameters.
  ...
  We are still using "older_than" approach in ADBlock-Private-DNS UI, but we realize that it’s easier to use offset/limit so here is this option now.
...
If ADBlock-Private-DNS’s web user is password-protected, a web client must use authentication mechanism when sending requests to server.  Basic access authentication is the most simple method - a client must pass `Authorization` HTTP header along with all requests:
