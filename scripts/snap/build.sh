...
version="$(./ADBlock-PD_amd64 --version | cut -d ' ' -f 4)"
if [ "$version" = '' ]; then
	log 'empty version from ./ADBlock-PD_amd64'
...
for arch in \
	...
	'armhf' \
	'i386'; do
	build_output="./ADBlock-PD_${arch}"
	snap_output="./ADBlock-PD_${arch}.snap"
	snap_dir="${snap_output}.dir"

	# Create the meta subdirectory and copy files there.
	mkdir -p "${snap_dir}/meta"
	cp "$build_output" "${snap_dir}/ADBlock-PD"
