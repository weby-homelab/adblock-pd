...
# ADBlock-Private-DNS Installation Script
	...
	*)
	...
	for cmd in $required; do
		...
		if ! is_command "$cmd"; then
			...
			error_exit "$cmd is required to install ADBlock-Private-DNS via this script"
		fi
	done
}
...
#
# See https://github.com/AdguardTeam/ADBlock-PD/issues/2443.
fix_darwin() {
	if [ "$os" != 'darwin' ]; then
		return 0
	fi
	...
	# It is important to install ADBlock-Private-DNS into the /Applications directory
...
set_download_func() {
	if is_command 'curl'; then
		...
		error_exit "either curl or wget is required to install ADBlock-Private-DNS via this script"
	fi
}
...
configure() {
	...
	pkg_name="ADBlock-PD_${os}_${cpu}.${pkg_ext}"
	url="https://static.adtidy.org/adguardhome/${channel}/${pkg_name}"
	agh_dir="${out_dir}/ADBlock-PD"
	readonly pkg_name url agh_dir

	log "ADBlock-Private-DNS will be installed into $agh_dir"
}
...
is_root() {
	...
	if is_command "$sudo_cmd"; then
		log 'note that ADBlock-Private-DNS requires root privileges to install using this script'

		return 1
	fi

	error_exit 'root privileges are required to install ADBlock-Private-DNS using this script
...
rerun_with_root() {
	script_url='https://raw.githubusercontent.com/AdguardTeam/ADBlock-PD/master/scripts/install.sh'
	readonly script_url
...
handle_existing() {
	...
	if [ "$existing_adguard_home" != '' ]; then
		log 'the existing ADBlock-Private-DNS installation is detected'

		if [ "$reinstall" -ne '1' ] && [ "$uninstall" -ne '1' ]; then
			error_exit \
				"to reinstall/uninstall the ADBlock-Private-DNS using this script specify one of the '-r' or '-u' flags"
		fi

		# TODO(e.burkov):  Remove the stop once v0.107.1 released.
		if (cd "$agh_dir" && ! ./ADBlock-PD -s stop || ! ./ADBlock-PD -s uninstall); then
			...
			log "cannot uninstall ADBlock-Private-DNS from $agh_dir"
		fi

		rm -r "$agh_dir"

		log 'ADBlock-Private-DNS was successfully uninstalled'
	fi
...
install_service() {
	...
	if (cd "$agh_dir" && maybe_sudo ./ADBlock-PD -s install); then
		return 0
	fi
	...
	#
	# See https://github.com/AdguardTeam/ADBlock-PD/issues/2542.
	if [ "$cpu" = 'armv7' ]; then
		cpu='armv5'
		reinstall='1'
	...
	error_exit 'cannot install ADBlock-PD as a service'
}
...
echo 'starting ADBlock-Private-DNS installation script'
...
printf '%s\n' \
	'ADBlock-Private-DNS is now installed and running' \
	'you can control the service status with the following commands:' \
	"$sudo_cmd ${agh_dir}/ADBlock-PD -s start|stop|restart|status|install|uninstall"
