...
# ADBlock-Private-DNS Release Script
#
...
log 'starting to build ADBlock-Private-DNS release'
...
build() {
	...
	if [ "$build_os" = 'windows' ]; then
		build_output="./${build_dir}/ADBlock-PD.exe"
	else
		build_output="./${build_dir}/ADBlock-PD"
	fi
	...
	'darwin' | 'windows')
		...
		# zip.
		(cd "${dist}/${1}" && zip -9 -q -r "../../${build_archive}" "./ADBlock-PD")
		;;
	*)
		build_archive="./${dist}/${build_ar}.tar.gz"
		tar -C "./${dist}/${1}" -c -f - "./ADBlock-PD" | gzip -9 - >"$build_archive"
		;;
	esac
...
echo "$platforms" | while read -r os arch arm mips; do
	...
	case "$arch" in
	arm)
		dir="ADBlock-PD_${os}_${arch}_${arm}"
		ar="ADBlock-PD_${os}_${arch}v${arm}"
		;;
	mips*)
		dir="ADBlock-PD_${os}_${arch}_${mips}"
		ar="$dir"
		;;
	*)
		dir="ADBlock-PD_${os}_${arch}"
		ar="$dir"
		;;
	esac
...
build_archive="./${dist}/ADBlock-PD_frontend.tar.gz"
...
if [ "$channel" = 'edge' ]; then
	announcement_url='https://github.com/AdguardTeam/ADBlock-PD/wiki/Platforms'
else
	announcement_url="https://github.com/AdguardTeam/ADBlock-PD/releases/tag/${version}"
fi
...
echo "{
  \"version\": \"${version}\",
  \"announcement\": \"ADBlock-Private-DNS ${version} is now available!\",
...
echo "
  \"download_linux_mips64\": \"${version_download_url}/ADBlock-PD_linux_mips64_softfloat.tar.gz\",
  \"download_linux_mips64le\": \"${version_download_url}/ADBlock-PD_linux_mips64le_softfloat.tar.gz\",
  \"download_linux_mipsle\": \"${version_download_url}/ADBlock-PD_linux_mipsle_softfloat.tar.gz\",
" >>"$version_json"
...
for f in $ar_files; do
	...
	# Remove the prefix.
	platform="${platform#"./${dist}/ADBlock-PD_"}"
